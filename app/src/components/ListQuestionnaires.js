...
export const ListQuestionnaires = ({ rol }) => {
    ...
    const [questionnaires, setQuestionnaries] =
    useState ([]);
    ...
    const getData = async ()=>{
        try {
            const {data} = await axios.get('http://localhost:4001/questionnarie/get-all');
            setQuestionnaries(data.questionnaires);
        } catch (error) {
            console.log(error)
            alert('Hubo un error al obtener cuestionarios')
        }
    }
